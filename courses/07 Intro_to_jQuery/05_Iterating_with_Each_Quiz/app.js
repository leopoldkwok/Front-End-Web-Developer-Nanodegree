/*
For this quiz, use jQuery's each() method to iterate through the <p>s,
calculate the length of each one, and add each length to the end of each <p>.

Also, make sure you don't change the text inside each <p> except to add the length, otherwise your
length numbers won't be correct!
*/

// Your code goes here!

$(document).ready(function(){

	$('p').each(function(index) {
		var iTotalWords = $(this).text().split('').length;
		$(this).append("<b> " + iTotalWords + "</b>");

	});

});

// alternative solution

// function numberAdder() {
// 	var text, number;

// 	text = $(this).text(); // finds the text

// 	number = text.length; // finds out how long it is

// 	$(this).text(text + " " + number); // changes the text on each element to the same text and a number

// }

// $('p').each(numberAdder);

